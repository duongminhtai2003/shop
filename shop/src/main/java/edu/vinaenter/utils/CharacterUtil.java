...
import edu.vinaenter.constants.CharacterDefine;
...
public class CharacterUtil {
	...
	public static String show() {
		...
		for (int i = 0; i < CharacterDefine.CHARS.length; i++) {
			str = (i != (CharacterDefine.CHARS.length - 1) ) 
					? str.concat(CharacterDefine.CHARS[i]).concat(", ")
					: str.concat(CharacterDefine.CHARS[i]);
		}
		return str;
	}
}
